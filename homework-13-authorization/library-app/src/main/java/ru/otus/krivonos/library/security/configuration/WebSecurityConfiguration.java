...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.csrf().disable()
			...
			.and()
			.authorizeRequests().antMatchers("/library/book/*").hasAnyRole("ADMIN", "USER")
			.and()
			...
			.and()
			.logout()
			.logoutUrl("/library/logout").permitAll().invalidateHttpSession(true).clearAuthentication(true).deleteCookies("JSESSIONID")
			...
			.and()
			.rememberMe().key("MySecret").tokenValiditySeconds(864_000);
	}
